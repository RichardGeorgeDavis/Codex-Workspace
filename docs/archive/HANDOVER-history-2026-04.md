...
- it does not currently run MemPalace closeout after repo intake because `tools/bin/workspace-memory` is temporarily disabled
...
### Temporary workspace-memory pause (2026-04-27)

`tools/bin/workspace-memory` is disabled for now. All MemPalace closeout, ingest,
search, wake-up, export, and graph commands intentionally fail fast while the
write-lock and corpus-size behavior is reviewed.

Current closeout expectation during the pause:

- record repo-specific closeout in tracked repo docs such as `README.md`, `HANDOVER.md`, or `DESIGN.md`
- record workspace-level closeout in `docs/HANDOVER.md` and `docs/CHANGELOG.md`
- use generated side-load summaries under `cache/context/` only as optional local context, not as canonical memory
- do not run or recommend `tools/bin/workspace-memory save-repo` or `tools/bin/workspace-memory save-workspace` until the pause is explicitly lifted

Reason for the pause: repeated repo closeout attempts hit the MemPalace write
lock while a live `save-repo` was embedding a large mirrored reference corpus.
The wrapper was hardened with broader excludes and longer lock diagnostics, but
the service remains paused until the closeout contract is reviewed.
...
Historical closeout expectation, currently paused:

- repo work previously used `tools/bin/workspace-memory save-repo <repo-name>`
- workspace-doc or planning work previously used `tools/bin/workspace-memory save-workspace`
- readable transcript bundles previously used `tools/bin/workspace-memory export-codex current`
- mixed repo and workspace closeout previously ran serially to avoid local-store contention
- explicit handover updates previously triggered automatic MemPalace closeout
...
These commands are paused and should not be run until the `workspace-memory`
disable block is removed.

Current operator entry points:

- use tracked docs and optional generated context-cache summaries for closeout context
- do not use the Workspace Hub memory command actions or shell wrappers until the pause is lifted
