...
- any future workspace memory service is a retrieval helper, not the source of truth
...
The workspace may use `tools/scripts/local-model-context.sh` as a task-time
local-first preparation worker. It extends the filesystem-first cache model; it
is not a memory service, authority layer or background indexer. Ollama handles
private bounded work; Gemini API is limited to explicitly selected tracked
public files. The worker receives deterministic, bounded source selections and
writes only disposable summaries, structured extraction and provenance under
`cache/context/local-models/`. See [24-local-model-context-handover.md](24-local-model-context-handover.md)
for commands, provider routing, redaction and protected-path rules.
...
## Tracked context catalogs

A repo-local `CONTEXT_CATALOG.md`, or an established index such as
`library/_catalog/index.md`, is a tracked human-reviewed task router. It maps
recurring questions to the smallest authoritative files without duplicating
source or current status.

Use `tools/scripts/context-catalog.sh <repo-path>` to preview an adaptive
location and add `--run` to create it. Existing catalog systems are preserved.
Catalogs are durable navigation, `cache/context/` summaries are disposable
side-loads, and Graphify is an ignored structural code graph. All may coexist,
but none is authoritative over source, tests, manifests, or handovers.
