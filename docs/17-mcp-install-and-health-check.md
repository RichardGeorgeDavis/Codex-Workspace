...
- refuses to replace a managed block that contains unrelated TOML tables or
  unbalanced markers, so it cannot silently remove personal Codex settings
- removes any older validated Codex Workspace MCP block from the active Codex config
