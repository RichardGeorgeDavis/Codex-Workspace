...
10. Classify Graphify eligibility as `code`, `documents`, `mixed`, `assets`, or `sensitive`; record why it is enabled, deferred, or excluded.
11. If README, HANDOVER, `DESIGN.md`, or other durable setup docs were created or materially updated, record the closeout in tracked docs.

### Graphify intake gate

Graphify is optional local code intelligence, not a default install step. Repo
intake may report readiness, but it must not create an allowlist, generate a
graph, install Codex integration, or add Git hooks automatically.

### Optional low-token context catalog

Assess whether repeated chat startup or file navigation warrants a tracked,
human-reviewable context catalog. This is often a better fit than Graphify for
document-heavy, mixed, private, operational, or loosely structured repos.

Preview first with `tools/scripts/context-catalog.sh <repo-path>`. Create it
only after reviewing the proposed location with
`tools/scripts/context-catalog.sh --run <repo-path>`.

The wrapper adapts to established repo structure: `00-ai-context/` first,
then an existing catalog hierarchy, `docs/`, or the repo root. It detects
standard existing catalogs, including `library/_catalog/index.md`, and does not
create a competing catalog. Use `--output <repo-relative-path>` only for a
deliberate alternative convention.

Review and refine the generated startup order, task router, tags, and folder
descriptions against the repo's actual authority model. A context catalog is
tracked navigation, Graphify is an ignored local structural graph, and
`cache/context/` contains disposable side-loads. None replaces source, tests,
manifests, status records, or tracked handovers.

Before adoption:

1. Decide whether the corpus is code-only or requires semantic document processing.
2. Review the processing boundary; document extraction requires a separately approved local or API model backend.
3. Add a self-contained `.graphifyignore` allowlist and an explicit `graphify-out/` rule in the repo `.gitignore`.
4. Run `tools/scripts/graphify-repo.sh <repo-path>` before using `--run`.
5. Inspect generated output for excluded paths, credentials, private content, absolute user paths, and hallucinated references.
6. Add Codex integration and local hooks only after the measured privacy and correctness gate passes.
7. Record the result in `docs/graphify-repo-adoption-register.md`.
...
5. a Codex-friendly closeout note in tracked repo docs
