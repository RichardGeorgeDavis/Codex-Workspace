...
- keep private skill-review destinations in operator configuration rather than
  hard-coding them into the public workspace contract
...
The public TomeVault manifest contains only generic framework skills:

- agent-tooling diagnosis
- repo onboarding and runtime triage
- workspace maintenance and release checks
- placeholder-only SSH Keychain guidance
- generic quality, security and TypeScript review templates

`.agents/skills/` is the generated public mirror; `shared/skills/`, plugin
skills and `tools/templates/skills/` are its public sources. Private model
routing, operator services, real hosts/accounts, repo-specific operations and
retained workspace experiments must live outside this repository and must not
...
### Private skill review

Operators may retain final-skill review copies in a private knowledge or
operations repository. That destination is intentionally not part of this
public workspace contract or its sync manifests.
