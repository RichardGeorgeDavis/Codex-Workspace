...
- Do not run `tools/bin/workspace-memory`; Workspace memory is temporarily paused while MemPalace write-lock and corpus-size behavior is reviewed.
...
5. Skip Workspace memory command actions until the pause is lifted.
...
- Running MemPalace commands during the workspace-memory pause.
