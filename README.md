...
Codex Workspace is a local-first system for working across many standalone repositories as one coherent environment. It gives you shared discovery, runtime control, docs, generated context side-loads, and agent tooling without forcing those repos into a monorepo or one shared dependency tree.
...
- shared docs, generated context, and tooling can sit above the repos without flattening their runtime models

**Tool-agnostic context.** Whether you use Codex, Cursor, Claude, or mix assistants, the same tracked docs, manifests, optional generated side-load summaries under `cache/context/`, and Workspace Hub give you a stable place to resume work when you switch tools—without rebuilding context from scratch. The optional local-first handover worker adds bounded, provenance-backed preparation without replacing canonical source files. Its public default is local-only; optional external routes require an explicitly selected private profile and tracked public-safe inputs. See [docs/20-ai-context-side-load.md](docs/20-ai-context-side-load.md), [docs/24-local-model-context-handover.md](docs/24-local-model-context-handover.md), and [Context for agents](#context-for-agents) below.
...
- a bounded Loop Engineering control plane for scheduled triage, isolated draft worktrees, and independent verification
- optional, privacy-first Graphify code intelligence for reviewed per-repo orientation and handover
- a filesystem-first context model for docs, manifests, and agent guidance
- a task-time local-first context worker with explicit scope, redaction, provenance and public-only cloud routing
...
| `tools/` | Scripts, manifests, optional core-service plumbing, MCP helpers | Shared automation without merging unrelated installs |
| `cache/` | Shared stores (npm/pnpm, generated context, service artifacts) | Speed and repeatability without a monorepo `node_modules` |
| `shared/` | Workspace metadata and standards | Durable state that is not duplicated inside each repo |
| `repos/workspace-hub/` | Hub app (React + API) | Discovery, runtime control, manifests, previews, and capability visibility |
...
- Shared docs, generated context, and tooling can span the whole workspace without pretending the codebases are one project.
...
- Use [docs/24-local-model-context-handover.md](docs/24-local-model-context-handover.md) for bounded Ollama extraction, public-only Gemini profile routing, explicit free-only OpenRouter routing and local-model handovers.
- Use [docs/22-loop-engineering.md](docs/22-loop-engineering.md) for safe scheduled triage, isolated drafts, and independent verification.
- Use [docs/23-codebase-knowledge-graph.md](docs/23-codebase-knowledge-graph.md) for the optional local Graphify pilot and privacy boundary.
- Use [docs/09-new-repo-baseline.md](docs/09-new-repo-baseline.md) when adding or onboarding a repo.

Repo-specific launchers can also live under ignored local or private operator folders. Those launchers should keep the host on `127.0.0.1`, prefer a repo-specific port range, and step to the next free port instead of randomising the address. The tracked `tools/local/commands/Run Workspace Hub.command` remains the public example.
...
- Later: Broader capability promotion after the current public surfaces settle.
...
- keep any future core workspace services in `tools/` with durable per-user state in `shared/`
...
- [docs/22-loop-engineering.md](docs/22-loop-engineering.md)
- [docs/23-codebase-knowledge-graph.md](docs/23-codebase-knowledge-graph.md)
- [docs/24-local-model-context-handover.md](docs/24-local-model-context-handover.md)
...
- optional private skill-review copies kept outside the public distribution tree
...
For repos with recurring navigation costs, `tools/scripts/context-catalog.sh`
can preview or create a tracked, human-reviewed task router. Existing catalog
systems are preserved, and routed source remains authoritative.
...
`ref/`, `screenshots/`, and `cache/` unless the task
...
The previous workspace memory runtime has been removed. Closeout now belongs in
tracked docs and optional generated context-cache summaries.
